...
public class Simulation{
	...
	public static void main(String[] args) throws IOException {
		...
		int numJobs = Integer.parseInt(sc.nextLine());
		...
		for (int count = 1; count < numJobs; count++) {
			...
			while (finishedJobs.length() != numJobs) {
				
				for (int i = 0; i < count; i++) {
					if (!processorQueue[i].isEmpty()) {
						B = (Job) processorQueue[i].peek();
						if (B.getFinish() - time == 0) {
			...
			for (int i = 1; i <= numJobs; i++) {
				Job back2default = (Job) processor.dequeue();
				back2default.resetFinishTime();
				tempStorage.enqueue(back2default);
				processor.enqueue(back2default);
			}
